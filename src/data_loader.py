...
def _ensure_exists(p: Path, kind="file"):
    """Raise FileNotFoundError if expected file/folder does not exist."""
    if not p.exists():
...
def merge_team_data(stats_df: pd.DataFrame, records_df: pd.DataFrame) -> pd.DataFrame:
    """Merge team stats and records into one DataFrame."""
...
def load_draft(path: str | Path = DRAFT_FILE) -> pd.DataFrame:
    """Load, clean, and aggregate draft data to team-season level."""
    p = Path(path); _ensure_exists(p)
    df = pd.read_csv(p)
    ...
    # Build full grid so every team-season is represented
...
def load_payroll(path: str | Path = PAYROLL_FILE) -> pd.DataFrame:
    """Load payroll data and reshape into long format."""
    p = Path(path); _ensure_exists(p)
    df = pd.read_csv(p)
...
def merge_front_office(
    ...
    """Merge coaches, draft, and payroll data into one DataFrame."""
    front_office_df = pd.merge(coach_df, draft_df, on=["Team", "Season"], how="inner")

    # Add number of distinct coaches per season
    coach_counts = (
    ...
    front_office_df = pd.merge(front_office_df, coach_counts, on=["Team", "Season"], how="left")

    if team_payroll_df is not None:
        front_office_df = pd.merge(front_office_df, team_payroll_df, on=["Team", "Season"], how="left")
...
def load_players(path: str | Path = PLAYER_STATS_DIR) -> pd.DataFrame:
    """Load and combine all player-stats CSVs into one DataFrame."""
...
def calculate_player_features(players_df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate player stats into team-season level features."""
    df = players_df[~players_df["Team"].isin(["2TM", "3TM", "4TM"])].copy()
    df["Team"] = df["Team"].map(team_map).fillna(df["Team"])

    # Production score
    df["Production_Score"] = (df["PTS"] + df.get("PLUS_MINUS", 0)) / df["MP"].replace(0, 1)
    ...
    return (
        ...
        .apply(lambda x: pd.Series({
            ...
            "injury_rate": (82 * len(x) - x["G"].sum()) / (82 * len(x))
        }))
        .reset_index()
    )
